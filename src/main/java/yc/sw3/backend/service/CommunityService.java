...
public class CommunityService {
    ...
    private PostDto.Response convertToDto(Post post) {
        return PostDto.Response.builder()
                ...
                .authorId(post.isAnonymous() || post.getAuthor() == null ? null : post.getAuthor().getId())
                .authorName(post.isAnonymous() ? "익명" : (post.getAuthor() != null ? post.getAuthor().getName() : "탈퇴한 사용자"))
                .category(post.getCategory())
                .isAnonymous(post.isAnonymous())
                .isPinned(post.isPinned())
                .createdAt(post.getCreatedAt())
                .comments(post.getComments() == null ? java.util.Collections.emptyList() : post.getComments().stream()
                        .map(c -> PostDto.CommentResponse.builder()
                                .id(c.getId())
                                .content(c.getContent())
                                .authorId(c.getUser() != null ? c.getUser().getId() : null)
                                .authorName(c.getUser() != null ? c.getUser().getName() : "탈퇴한 사용자")
